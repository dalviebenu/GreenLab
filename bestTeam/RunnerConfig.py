...
class RunnerConfig:
    ...
    def __init__(self):
        ...
        self.ssh_host = "145.108.225.16"
        self.ssh_username = "teambest"
        self.ssh_password = "greenandgood"
        self.ssh_port = 22  # Default SSH port
        self.ssh_client = None  # Paramiko client instance
    ...
    def create_run_table_model(self) -> RunTableModel:
        ...
        # governor_types = ["schedutil", "performance", "powersave", "userspace", "ondemand", "conservative"]
        governor_types = ["schedutil"]
        # workload_types = [0, 1, 2]
        workload_types = [0]

        # Create the factor models ensuring no duplicates
        governor_type_factor = FactorModel('governor_type', governor_types)
        workload_type_factor = FactorModel('workload_type', workload_types)

        self.run_table_model = RunTableModel(
            factors=[governor_type_factor, workload_type_factor],
            # factors=[workload_type_factor],
            # repetitions=10,
            # shuffle=True,
            data_columns=[
                'governor_type',
    ...
    def before_run(self) -> None:
        ...
        """Perform any activity required before starting a run."""
        if self.run_table_model:
            current_run = self.run_table_model.generate_experiment_run_table()
            print("current run elements:", current_run)
            self.governor_type = current_run["governor_type"]
            self.workload_type = current_run["workload_type"]
            output.console_log(f"Selected Governor: {self.governor_type}, Workload: {self.workload_type}")
        else:
            raise ValueError("Run table model is not initialized.")


        if self.run_table_model:
            self.workload_type = self.run_table_model.get_data_columns()[self.current_run_index]
            output.console_log(f"Workload type set to: {self.workload_type}")
        else:
            output.console_log("Run table model is not initialized.")
    ...
    def interact(self, context: RunnerContext) -> None:
        ...
        # Define workload experiments (example)
        experiments = [
            {"threads": 100, "connections": 1000},
            {"threads": 100, "connections": 5000},
            {"threads": 100, "connections": 10000},
        ]

        print("workload type interact:", self.workload_type)

        # Define the working directory and the wrk command
        wrk_command = (
            f"sshpass -p \"{self.ssh_password}\" ssh {self.ssh_username}@{self.ssh_host} "
            ...
            f"http://{self.ssh_host}:8080/wrk2-api/post/compose -R 2\'"
        )
